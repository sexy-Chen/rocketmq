...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * </li>
     * </ul>
     * 从何处开始消费 --- 消息进度从消息服务器拉取不到时,重新计算消费策略
     */
     ...
     * 并发消息消费时处理队列最大跨度,表示如果消息处理队列中偏移量最大的消息和偏移量最小的消息
     * 的跨度超过2000则延迟50ms之后再拉取消息
     */
     ...
     * 推模式下拉取任务的间隔时间,默认一次拉取任务完成继续拉取
     */
    private long pullInterval = 0;

    /**
     * Batch consumption size
     * 消息并发消费时一次消费消息条数, --- 每次传入MessageListener#consumer中的消息条数
     */
     ...
     * queue waiting.
     * 最大消费重试次数,如果消息消费次数超过maxReconsumeTimes还未成功,将会放到失败队列,等待被删除
     */
